...
public class FallenTreeFamily extends MultiConnectFamily {
    ...
    public FallenTreeFamily(BlockFamilyDefinition definition, BlockBuilderHelper blockBuilder) {
        ...
        this.registerBlock(blockUri, definition, blockBuilder, "middle", SideBitFlag.getSides(Side.LEFT, Side.RIGHT), Rotation.horizontalRotations());
        this.registerBlock(blockUri, definition, blockBuilder, "lone", (byte) 0, Rotation.horizontalRotations());
    ...
    protected boolean connectionCondition(Vector3i blockLocation, Side connectSide) {
        Vector3i neighbourPos = new Vector3i(blockLocation);
        neighbourPos.add(connectSide.getVector3i());
        return worldProvider.isBlockRelevant(neighbourPos) && worldProvider.getBlock(neighbourPos).getBlockFamily() instanceof FallenTreeFamily;
    }

    @Override
    public byte getConnectionSides() {
        return SideBitFlag.getSides(Side.LEFT, Side.RIGHT);
    }
